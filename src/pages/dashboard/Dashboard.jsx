...
const Dashboard = () => {
  ...
  const buildAlertsFromProducts = useCallback((products) => {
    const stock = [];
    const expire = [];
    ...
    expire.sort((a, b) => {
      const oa = a.type === "expired" ? 0 : a.type === "expire_critical" ? 1 : 2;
      const ob = b.type === "expired" ? 0 : b.type === "expire_critical" ? 1 : 2;
  ...
  const fetchRecentActivity = useCallback(async () => {
    try {
      ...
      const sales =
        salesRes.status === "fulfilled" ? extractSales(salesRes.value) : [];
      const purchases =
        purchasesRes.status === "fulfilled"
          ? extractPurchases(purchasesRes.value)
          : [];
  ...
  const refreshAlertsOnly = useCallback(async () => {
    try {
      ...
      buildAlertsFromProducts(products);
    } catch (error) {
      console.error("Error fetching stock/expire alerts:", error);
      setStockAlerts([]);
      setExpireAlerts([]);
    }
  }, [buildAlertsFromProducts]);
    ...
    try {
      ...
      try {
        ...
        if (response?.success) {
          ...
          setStats((prev) => ({
            ...prev,
            totalProducts: toNumber(dashboardStats.totalProducts),
            totalSales: toNumber(dashboardStats.totalSales),
            totalPurchases: toNumber(dashboardStats.totalPurchases),
            totalCustomers: toNumber(dashboardStats.totalCustomers),
            totalRevenue: toNumber(dashboardStats.totalRevenue),
            totalStock: toNumber(dashboardStats.totalStock),
            outOfStock: toNumber(dashboardStats.outOfStock),
            lowStock: toNumber(dashboardStats.lowStock),
            expiringSoon:
              dashboardStats.expiringSoon !== undefined
                ? toNumber(dashboardStats.expiringSoon)
                : prev.expiringSoon,
            expired:
              dashboardStats.expired !== undefined
                ? toNumber(dashboardStats.expired)
                : prev.expired,
          }));

          setProductSalesData(Array.isArray(productSales) ? productSales : []);

          await Promise.all([fetchRecentActivity(), refreshAlertsOnly()]);
          return;
        }
      } catch (dashboardError) {
        console.error("Error fetching dashboard API:", dashboardError);
      }
      ...
      const products =
        productsRes.status === "fulfilled"
          ? extractProducts(productsRes.value)
          : [];
      const sales =
        salesRes.status === "fulfilled" ? extractSales(salesRes.value) : [];
      const purchases =
      ...
      const customers =
        customersRes.status === "fulfilled"
          ? extractCustomers(customersRes.value)
          : [];
  ...
  }, [buildAlertsFromProducts, fetchRecentActivity, refreshAlertsOnly]);
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
          <div className="xl:col-span-8">
            <div className="rounded-xl bg-white border-2 border-gray-200 shadow-sm p-5 h-full">
              ...
              <div className="w-full min-w-0" style={{ height: 450 }}>
                <ProductSalesChart data={productSalesData} />
              </div>
            </div>
          </div>
                  ...
                  recentActivity.map((a, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`flex-shrink-0 h-9 w-9 rounded-lg flex items-center justify-center text-white ${
                            a.type === "sale" ? "bg-emerald-500" : "bg-violet-500"
                          }`}
                        >
                          {a.type === "sale" ? (
                            <ShoppingCart className="h-4 w-4" />
